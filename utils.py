...
def make_metaworld_env(cfg):
    ...
    if env_name in _env_dict.ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE:
        env_cls = _env_dict.ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE[env_name]
    else:
        env_cls = _env_dict.ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE[env_name]
    
    if cfg.save_video:
        env = env_cls(render_mode="rgb_array")
        env.model.vis.global_.offwidth = DEFAULT_SIZE
        env.model.vis.global_.offheight = DEFAULT_SIZE
        env.mujoco_renderer = MujocoRenderer(env.model, env.data, DEFAULT_CAMERA_CONFIG, DEFAULT_SIZE, DEFAULT_SIZE)
        # env = env_cls(render_mode='rgb_array')
    else:
        env = env_cls()
...
def make_metaworld_env_fix(cfg):
    env_name = cfg.env.replace('metaworld_','')
    env_name = env_name + "-goal-observable"
    # if env_name in _env_dict.ALL_V2_ENVIRONMENTS:
    #     env_cls = _env_dict.ALL_V2_ENVIRONMENTS[env_name]
    ...
    env.seed(cfg.seed)
    env._freeze_rand_vec = True  # fix init
    env._set_task_called = True
    # env.seed(cfg.seed)
...
def save_numpy_as_gif(array, filename, fps=20, scale=1.0):
    ...
    if array.ndim == 3:
        ...
        # array = np.array([np.flipud(frame) for frame in array])   #解决metaworld图像反转问题
